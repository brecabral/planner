...
4. Definir o resultado observável e os efeitos sobre dados; deixar escolhas de implementação livres quando não alterarem o contrato.
5. Manter `draft` enquanto houver dúvidas bloqueantes. Registrar a evidência das decisões antes de marcar `ready`; não presumir aprovação pela passagem do tempo.
6. Atualizar o índice de specs e os links pertinentes. Entregar o caminho da spec, dúvidas restantes e critérios que permitirão implementar. Não criar código ou iniciar outra tarefa por consequência automática.
