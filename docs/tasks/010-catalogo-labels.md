...
# 010 — Gerar o catálogo privado de labels

## Entrada e limite

[SPEC-006](../specs/006-labels.md) RN01 e RN03; consumir authenticated-user-scope. Referências Phoenix e Ecto.

## Scaffold

```sh
mix phx.gen.context Labels Label labels name:string --scope user
```
...
Manter schema, migration, criação/consulta e fixtures scoped gerados. Ajustar trim e validação de nome. Retirar update/delete não previstos; não criar interface CRUD de labels. Oferecer operação componível em transação externa, sem commit independente.
